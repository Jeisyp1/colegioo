...
export class PortalComponent implements OnInit {
  ...
  llegue(): void {
    ...
    if (alumnosASubir.length > 0) {
      ...
      // Limpiar la tabla local después de enviar los alumnos
      this.limpiarTablaLocal();
    } else {
      console.warn('No hay alumnos para enviar.');
    }
  }
  
  limpiarTablaLocal(): void {
    this.alumnosGuardadosLocalmente = [];
    this.organizarAlumnosPorCarril();
  }
  ...
  enviarAlumnos(): void {
    ...
    this.alumnosGuardadosLocalmente = []; // Limpiar los datos locales
    this.organizarAlumnosPorCarril();
  }
